...
<div class="IG-block" id="block_{row.id}">
  {#if row.storyURL}
    <a href="{row.storyURL}">
      <div
        class="img-wrapper"
        on:mouseenter|self="{toggleVisible}"
        on:mouseleave="{toggleVisible}"
        bind:clientWidth="{w}"
        style="height: {w}px"
      >
        {#if imageIndex < 3}
        ...
        {/if}
        {#if visible}
          <p transition:fly="{{ y: w, duration: 500 }}" class="IG-hed">
            {row.hed}
          </p>
        {/if}
      </div>
